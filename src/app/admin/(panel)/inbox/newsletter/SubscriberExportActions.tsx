...
export function SubscriberExportActions({ subscribers }: SubscriberExportActionsProps) {
  ...
  async function handleCopy() {
    try {
      ...
      setStatus({
        kind: 'success',
        message: `Copied ${subscribers.length} active email ${subscribers.length === 1 ? 'address' : 'addresses'}.`,
      })
    } catch (error) {
      setStatus({
        kind: 'error',
  ...
  function handleExport() {
    try {
      ...
      setStatus({
        kind: 'success',
        message: `Downloaded ${subscribers.length} active ${subscribers.length === 1 ? 'subscriber' : 'subscribers'}.`,
      })
    } catch {
