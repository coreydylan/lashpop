...
async function triggerJob(job: DatasetJob, apiToken: string) {
...
async function fetchText(url: string, apiToken: string) {
...
async function downloadSnapshotData(job: DatasetJob, info: { snapshotId?: string; statusUrl?: string }, apiToken: string) {
  ...
  for (let attempt = 0; attempt < 120; attempt++) {
    const raw = await fetchText(snapshotUrl, apiToken)
    ...
    try {
      ...
      if (status && ['done', 'success', 'completed', 'finished'].includes(status)) {
        if (resultUrl && !isSourceUrl(resultUrl, job)) {
          return await fetchText(resultUrl, apiToken)
        }
        return await fetchText(downloadUrl, apiToken)
      }
...
export async function syncBrightDataReviews() {
  ...
  if (!jobs.length) {
    console.log('No BrightData datasets configured')
    return { sources: [] }
  }
  ...
  for (const job of jobs) {
    console.log(`Triggering BrightData dataset for ${job.source} (${job.datasetId})`)
    const triggerInfo = await triggerJob(job, BRIGHTDATA_API_TOKEN)
    let rawData: string
    if (triggerInfo.resultUrl && !isSourceUrl(triggerInfo.resultUrl, job)) {
      ...
      rawData = await fetchText(triggerInfo.resultUrl, BRIGHTDATA_API_TOKEN)
    } else {
      rawData = await downloadSnapshotData(job, triggerInfo, BRIGHTDATA_API_TOKEN)
    }
    ...
    const syncResult = await syncReviews(normalized)
    results.push({
      source: job.source,
      parsed: normalized.length,
      inserted: syncResult.insertedRaw,
      testimonials: syncResult.insertedTestimonials
    })
  }
