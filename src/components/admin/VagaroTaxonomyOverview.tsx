      ...
      <div className="border-b border-sage/10 bg-gradient-to-r from-ocean-mist/10 via-white/40 to-dusty-rose/10 p-5 sm:p-6">
          ...
          <div className="flex gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="font-serif text-xl text-dune">Booking categories</h2>
                {hasWarning ? (
                  <TriangleAlert className="h-4 w-4 text-golden" aria-label="Some Vagaro categories are not linked to website categories" />
                ) : (
                  <CheckCircle2 className="h-4 w-4 text-ocean-mist" aria-label="All Vagaro categories are linked to website categories" />
                )}
              </div>
              <p className="mt-1 max-w-3xl text-sm leading-6 text-dune/65">
                Each Vagaro category is linked, or mapped, to a LashPop website category. Vagaro controls booking names, order, availability, services, and eligible stylists. Edit website labels, descriptions, and images here.
              </p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link href="/admin/system/syncs" className="btn btn-secondary text-xs">
              View Vagaro updates <ArrowRight className="h-3.5 w-3.5" />
            </Link>
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-3">
          <div className="rounded-2xl border border-ocean-mist/15 bg-white/70 p-4">
            ...
            <p className="text-xs font-semibold uppercase tracking-wider text-dune/45">Change in Vagaro</p>
            <p className="mt-1 text-sm text-dune/70">Category names, order, availability, services, and eligible stylists</p>
          </div>
          <div className="rounded-2xl border border-dusty-rose/15 bg-white/70 p-4">
            <LockKeyhole className="mb-2 h-4 w-4 text-dusty-rose" />
            <p className="text-xs font-semibold uppercase tracking-wider text-dune/45">Edit here</p>
            <p className="mt-1 text-sm text-dune/70">Website labels, descriptions, taglines, icons, and category images</p>
          </div>
          <div className="rounded-2xl border border-golden/15 bg-white/70 p-4">
            <Layers3 className="mb-2 h-4 w-4 text-golden" />
            <p className="text-xs font-semibold uppercase tracking-wider text-dune/45">Manage separately</p>
            <p className="mt-1 text-sm text-dune/70">Homepage service cards and the Botox category position</p>
          </div>
        </div>
      </div>

      <ul className="divide-y divide-sage/10 md:hidden" aria-label="Links between Vagaro and website categories">
          ...
          <li key={category.id} className="px-4 py-4 text-dune/70 sm:px-5">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-0.5 font-mono text-[10px] text-dune/35">Vagaro ID {category.vagaroCategoryId}</p>
              </div>
              <span className={`shrink-0 rounded-md px-2.5 py-1 text-[10px] font-semibold ${
                ...
                {category.isActive ? (category.mappedCategoryName ? 'Linked' : 'Needs linking') : 'Removed from Vagaro'}
              </span>
            </div>
            ...
            <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3">
              ...
              <div className="col-span-2">
                <dt className="text-[10px] font-semibold uppercase tracking-wider text-dune/45">Website category</dt>
                  ...
                  {category.mappedCategoryName ? (
                    <span className="flex flex-wrap items-center gap-2">
                      ...
                      {category.mappedCategorySlug === 'lashes' && category.title === 'Lash Lifts' && (
                        <span className="rounded-md bg-ocean-mist/10 px-2 py-0.5 text-[10px] font-medium text-ocean-mist">Combined with Lashes</span>
                      )}
                    </span>
                  ) : (
                    <span className="font-medium text-golden">Waiting to link</span>
                  )}
                </dd>
              </div>
              <div className="col-span-2">
                <dt className="text-[10px] font-semibold uppercase tracking-wider text-dune/45">Stylist profile label</dt>
                <dd className="mt-1 text-sm text-dune/75">
        ...
        {localBookingCategories.map(category => (
          <li key={category.id} className="bg-dusty-rose/[0.035] px-4 py-4 text-dune/70 sm:px-5">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <p className="text-sm font-semibold text-dune">LashPop-only category</p>
                <p className="mt-0.5 text-[10px] text-dune/35">Vagaro updates do not change it</p>
              </div>
            ...
            <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-3">
              ...
              <div className="col-span-2">
                <dt className="text-[10px] font-semibold uppercase tracking-wider text-dune/45">Stylist profile label</dt>
                <dd className="mt-1 text-sm text-dune/75">Set on each team profile</dd>
              </div>
            </dl>
          </li>
        ))}
      ...
      <div className="hidden overflow-x-auto md:block">
        <table className="w-full min-w-[820px] text-left text-sm">
          <thead className="border-b border-sage/10 bg-cream/45 text-[11px] uppercase tracking-wider text-dune/45">
            <tr>
              <th className="px-5 py-3 font-semibold">Vagaro category</th>
              <th className="px-4 py-3 font-semibold">Services</th>
              <th className="px-4 py-3 font-semibold">Website category</th>
              <th className="px-4 py-3 font-semibold">Booking position</th>
              <th className="px-4 py-3 font-semibold">Stylist profile label</th>
          ...
          <tbody className="divide-y divide-sage/10">
            {rawCategories.map(category => (
              <tr key={category.id} className="text-dune/70">
                <td className="px-5 py-3.5">
                  ...
                  <div className="mt-0.5 font-mono text-[10px] text-dune/35">Vagaro ID {category.vagaroCategoryId}</div>
                </td>
                ...
                <td className="px-4 py-3.5">
                  {category.mappedCategoryName ? (
                    <div className="flex items-center gap-2">
                      <span>{category.mappedCategoryName}</span>
                      {category.mappedCategorySlug === 'lashes' && category.title === 'Lash Lifts' && (
                        <span className="rounded-full bg-ocean-mist/10 px-2 py-0.5 text-[10px] font-medium text-ocean-mist">Combined with Lashes</span>
                      )}
                    </div>
                  ) : (
                    <span className="font-medium text-golden">Waiting to link</span>
            ...
            {localBookingCategories.map(category => (
              <tr key={category.id} className="bg-dusty-rose/[0.035] text-dune/70">
                <td className="px-5 py-3.5">
                  <div className="font-medium text-dune">LashPop-only category</div>
                  <div className="mt-0.5 text-[10px] text-dune/35">Vagaro updates do not change it</div>
                </td>
                <td className="px-4 py-3.5">—</td>
                <td className="px-4 py-3.5">{category.name}</td>
                <td className="px-4 py-3.5 tabular-nums">#{category.displayOrder}</td>
                <td className="px-4 py-3.5">Set on each team profile</td>
                <td className="px-5 py-3.5 text-right">
