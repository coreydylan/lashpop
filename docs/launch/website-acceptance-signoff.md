...
| Analytics | Vercel Web Analytics and Speed Insights are the privacy-friendly launch baseline. The Pro project supports custom events; booking, quiz, Work With Us, and newsletter events are restricted to two non-PII properties. GTM and Meta identities remain dormant behind `NEXT_PUBLIC_MARKETING_TRACKING_ENABLED` until LashPop requests advertising and approves the related consent/GPC behavior | Candidate test and production event proof required |
