...
| --- | --- | --- |
| Candidate | Main `f36ac74f52bb83eff332c9a48f786d62b85e0c8a` from PR #40; Vercel production `dpl_7Xs1XpwVqmJHn9nr6gNjtjqv9UgE`; Vagaro sync Worker `5544ded0-9c80-4eba-b986-2c7c75296b55` | Source, CI, and platform deployment verified |
| Architecture | Public Next.js paths use `imagedelivery.net` directly. `lashpop-img` version `571076ec-2e5f-41c1-b16e-2639a94551ea` remains transition/rollback infrastructure only; no app loader, rewrite, or public raster path depends on it | Verified in production code and browser network evidence |
| Inventory and direct data | D1 migration `0012` is applied. The registry has 527/527 ready rows and 524 distinct image IDs: 456 first-party sources, 55 service sources, and 16 staff sources. Active/public Rackspace image delivery and missing provider sources are zero | Verified in production D1; `quick_check=ok` and foreign keys clean |
| Refresh behavior | Allow-listed source fetch, conditional refresh, content-hash idempotency, adoption without re-upload, immutable changed-content IDs, retained prior ID, last-ready preservation, and fail-closed first ingestion passed. Two authenticated production syncs completed 89/89 services and 15/15 staff; the second preserved image counts and registry IDs | Unit and real-data idempotency verified |
| Width/format delivery | Provider matrix 612/612, representative matrix 612/612, and independent browser matrix 153/153 passed. Transparent originals retain a correctly labeled native PNG only when a JPEG-only client cannot accept AVIF/WebP | Verified Cloudflare-native negotiation; no availability, dimension, host, or fallback failures |
| Browser/network | Exact Vercel production passed desktop, 390px, and 320px full-scroll checks; all four quiz outcomes; gallery; team; Mapbox; Classic Fill touch booking; and every public booking category. No broken images, overflow, public Worker/R2/Rackspace image requests, console errors, or Vercel error logs were found | Verified on the deployed candidate |
| Admin/privacy | Public action results replace source URLs with direct delivery URLs; source-only Vagaro fields are nulled at the public boundary. `/admin` redirects to login and unauthenticated DAM API returns 401. New raster storage writes fail closed unless Cloudflare Images ingestion succeeds; deletion covers source and delivery objects | Public boundaries verified in production; authorized admin workflows remain a human gate |
| Map | Mapbox canvas, tiles, marker, and controls passed at 1440px, 390px, and 320px on the exact Vercel candidate | Verified |
| Analytics | Vercel Web Analytics and Speed Insights are active and had recorded 638 aggregate visitors and 2,224 pageviews before the custom-event release. The Pro project now records privacy-safe booking, quiz, Work With Us, and newsletter events with at most two non-PII properties. Production GTM and Meta requests are zero; their identities remain dormant for a future approved advertising program | Exact production runtime and event contract verified |
| Vagaro webhook | Field/Vercel contain `VAGARO_WEBHOOK_SECRET`, but the exact deployment has no provider-attributable signed 200; observed POSTs returned 401 | Open external gate: align the Vagaro View Details Verification Token and run Vagaro's Test Webhook |
| Lead notification | Work With Us submissions persist to D1/Admin before notification. The legacy Mox notification server is unavailable, so email delivery is not accepted even though leads remain recoverable in Admin | Open external gate: provision the approved replacement provider and prove both recipients receive one controlled test |
| Deployment and DNS | Direct app, D1 data, Images secret, and Vagaro Worker are deployed with recorded rollback versions. `lashpop.vercel.app` serves the exact candidate. Public apex and `www` DNS were not changed and still serve Squarespace | Platform ready; public-domain convergence not started |
